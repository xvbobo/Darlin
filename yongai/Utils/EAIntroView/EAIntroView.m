...
- (void)buildScrollViewWithFrame:(CGRect)frame {
    ...
    for (int idx = 0; idx < _pages.count; idx++) {
        ...
        if(idx == _pages.count - 1){
            //最后一个View
            //初始化点击进入的按钮
//            if(viewFrame.size.height == 480)
//                enterButton = [[UIButton alloc] initWithFrame:CGRectMake(50, viewFrame.size.height - 70, viewFrame.size.width - 100, 50)];
//            else if(viewFrame.size.height ==568)
//                enterButton = [[UIButton alloc] initWithFrame:CGRectMake(50, viewFrame.size.height - 80, viewFrame.size.width - 100, 50)];
//            else
//                enterButton = [[UIButton alloc] initWithFrame:CGRectMake(50, viewFrame.size.height - 90, viewFrame.size.width - 100, 50)];
            
            enterButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0 , viewFrame.size.width , viewFrame.size.height)];
//            enterButton.backgroundColor = BJCLOLR;
            [enterButton addTarget:self action:@selector(skipIntroduction) forControlEvents:UIControlEventTouchUpInside];
            [pageViews[idx] addSubview:enterButton];
        }
    }
