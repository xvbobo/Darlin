...
CGFloat SVProgressHUDRingRadius = 14;  // 提示框弧度
...
+ (void)showErrorWithStatus:(NSString *)string andOther:(NSString *)string1
{
...
+ (void)showErrorWithFaBuStatus:(NSString *)string
{
   [[SVProgressHUD sharedView] showImage:[UIImage imageNamed:@"SVProgressHUD.bundle/error.png"] status:string duration:2.5];
}
...
{
    [[SVProgressHUD sharedView] showHugView];
}

#pragma mark - Instance Methods
- (void)showHugView
{
    hudView.backgroundColor = BJCLOLR;
...
- (void)showProgress:(float)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    NSLog(@"%lu",(unsigned long)self.maskType);
    NSString * str = [NSString stringWithFormat:@"%lu",(unsigned long)self.maskType];
